...
**Claim work by editing this file in the same commit.** Prefix the entry with
`[WIP <date>]` when you start and delete it when it lands. Two sessions doing the
same entry is the failure mode this document exists to prevent.
...
**Update the honesty register.** It lists claims this repo's own documents make
that are not true. If you make one of them true, delete the line in the same
commit. If you find a new one, add it.
